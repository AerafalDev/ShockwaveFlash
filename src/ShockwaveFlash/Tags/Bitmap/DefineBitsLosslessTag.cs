...
public sealed class DefineBitsLosslessTag : Tag
{
    ...
    public DefineBitsLosslessTag(TagMetadata metadata, ushort id, ushort width, ushort height, BitmapFormat format, ReadOnlyMemory<byte> zLibBitmapData) : base(metadata)
    {
        Id = id;
        Width = width;
        Height = height;
    ...
    public static DefineBitsLosslessTag Decode(MemoryReader reader, TagMetadata metadata)
    {
        ...
        return new DefineBitsLosslessTag(metadata, id, width, height, format, zlibBitmapData);
    }
